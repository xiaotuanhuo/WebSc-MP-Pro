...
var pickout = (function() {
	 ...
	 * 
	 * @param {[type]}
	 *            config = String or object to setting
	 */
	...
	function createElements(select, index) {
		...
		if (parent.hasAttribute('for'))
			input.setAttribute('id', parent.getAttribute('for'));
	...
	function fireModal(config) {
		...
		var modal = document.querySelector('.pk-modal'), select = config.DOM[config.currentIndex], data;

		// modal theme
		modal.setAttribute('class', modal.getAttribute('class') + ' -'
				+ config.theme);

		// Avoid charging again when changing tab and the field gives focus
		// again
		...
		var title = select.hasAttribute('placeholder') ? select
				.getAttribute('placeholder') : 'Select to option';
		...
		if (config.search) {
			...
			search.setAttribute('class', search.getAttribute('class')
					+ ' -show');
			...
			inputSearch.addEventListener('keyup', function(e) {
				...
				var optionsDefault = optionsModal, main = modal
						.querySelector('.main');
				...
				optionsModal.map(function(option) {
					...
					if (txt.innerHTML.toLowerCase().indexOf(
	 ...
	 *            data = {index, option}
	 * @param {object
	 *            DOM} modal = Modal that will receive the list
	 * @param {object}
	 *            config
	 */
	function createOption(data, modal, config) {
		...
		item.setAttribute('class', 'pk-option ' + selected + ' -'
				+ config.theme);
		item.setAttribute('data-id', data.item.getAttribute('value'));
		...
		item.addEventListener('click', function(e) {
			...
			feedInput(select, txt.innerHTML, item.getAttribute('data-id'));
			closeModal();
		}, false);
		return item;
	}
	
	function feedInput(select, value, id) {
	...
	/**
	 * Merges the settings passed by the user with the default settings of the
	 * package, adding their own configurations
	 */
