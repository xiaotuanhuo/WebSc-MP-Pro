(function($, window, document, undefined) {
    ...
    $(document).click(function(event) {
        ...
        if (!_con2.is(event.target) && !_con.is(event.target) && _con.has(event.target).length === 0) {
            $('.select-picker-search').addClass("open");
        }
    });
